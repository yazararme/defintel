...
## Attempt 2 (verdict `review/verdicts/rev-21-deneme1.md`: R21-P0-2 FAIL, evidence missing)

- **Evidence is now produced by the pipeline.** `scripts/check_reports.py --oyuncular` also opens the "Mühimmat" chip at 375 and 1440, in light and dark, and takes full-page shots `3-oyuncular-muhimmat-375.png`, `4-oyuncular-muhimmat-1440.png`, `5-oyuncular-muhimmat-375-koyu.png`, `6-oyuncular-muhimmat-1440-koyu.png` (1-2 are unchanged, Tümü). The CI step uploads `*.png`, so the `kapsam-sayi-<run>-<attempt>` artifact now contains all six. The (A) link text in `build.yml` names them.
- New checks per Mühimmat shot, in both states: "izlenen N" equals the visible row count and the number of players tagged `muhimmat` (23 of 64). Below 64 the top line must be exactly `izlenen N` with no count/age tokens. The Tümü shots below 64 now also check alphabetical order (using `build.py`'s own `tr_collate`, extracted via `ast` so no markdown/yaml import is needed).
- **Real defect fixed (reviewer's observation).** `.chip:hover` (two-part selector) overrode the one-class `.pchip--on`, so the active chip had light text on a light fill while the pointer or a sticky touch-hover stayed on it (measured contrast 1.1:1). `assets/app.css`: `.pchip--on, .pchip--on:hover, .pchip--on:active` keep the brand fill. The check clicks the chip, leaves the pointer on it, and requires ≥4.5:1 contrast: old CSS 🔴 1.1:1, new CSS 🟢 9.2:1 (light) / 8.4:1 (dark). All HTML diffs from this are only the `app.css?v=` hash.
- Local <64 evidence (`KAPSAM_BOZ=thales python3 build.py`, then `--oyuncular --out`): `review/shots/rev-21/after-alt64/1-…6-*.png`. With Mühimmat on, the top line reads `izlenen 23`, the first rows are Arsenal Bulgaria and BAE Systems, and there are 0 counted rows.
- `review/tools/smoke.py` also runs `--oyuncular` in the <64 state. Results: build 64/64; `--oyuncular` 6/6 🟢 in both states; DÖRT-DURUM 4/4; `test_uyari.py` 28/28; SMOKE OK. The tree is left in the normal state (64/64).
- Not done (limits): a new CI dispatch with `kapsam_boz=thales` to put the Mühimmat shots into a CI artifact. The next such run will contain them.
