...
| `scripts/fixtures/ceviri-ornegi-40.json` | new fixture: the §2 sample (public headlines + existing translations), so CI does not depend on the gitignored `audit/`. |
...
`audit/content.md` §2 names only **12 of the 16**: 8 are named in the text, plus "cümle düzenindeki 7 başlığın 7'si", with 3 overlapping. The per-item verdicts are not in the repo. The items and today's translations come from `scripts/fixtures/ceviri-ornegi-40.json`, a committed copy of the 40-item sample behind §2 with only source, language, original and translation (`audit/` is gitignored, so the first CI run failed on a FileNotFoundError), and the test re-derives the 8 named + 7 sentence-case items from it. The other 4 defects are somewhere in the remaining 28-item pool. So the test reports a **guaranteed lower bound**: it assumes the 4 unknowns are the pool items no check catches. It also reports an expected value.
